...
public static class SingletonUtility
{
	...
	public static T GetOrInitSingletonAsset<T>() where T : ScriptableObject
	{
		...
		{
			Debug.LogWarning($"{nameof(GetOrInitSingletonAsset)}: Could not get instance because the application state is changing");
			return null;
		}
		...
		if (TryLoadSingleton(out instance))
		{
			...
			Debug.Log($"{nameof(GetOrInitSingletonAsset)}: load from resources, type = {type.Name}", instance);
			return instance;
		}

		instance = ScriptableObject.CreateInstance<T>();
		Debug.Log($"{nameof(GetOrInitSingletonAsset)}: creating new instance, type = {type.Name}", instance);
		return instance;
	}
	...
	public static T GetOrInitSingletonBehaviour<T>() where T : MonoBehaviour
	{
		...
		if (_applicationStateIsChanging)
		{
			Debug.LogWarning($"{nameof(GetOrInitSingletonBehaviour)}: Could not get instance because the application state is changing");
			return null;
		}
		...
		if (instance != null)
		{
			...
			Debug.Log($"{nameof(GetOrInitSingletonBehaviour)}: found in scene, type = {typeof(T).Name}", instance);
			return instance;
		}
		...
		if (TryLoadSingleton<T>(out var prefab))
		{
			...
			SetSingleton(instance);
			Debug.Log($"{nameof(GetOrInitSingletonBehaviour)}: load from resources, type = {typeof(T).Name}", instance);
			return instance;
		}
		...
		Debug.Log($"{nameof(GetOrInitSingletonBehaviour)}: creating new instance, type = {typeof(T).Name}", instance);
		return instance;
	}
	...
	public static IEnumerator InitializeSingletonAsync<T>() where T : UnityObject
	{
		if (IsInitialized<T>())
		{
			Debug.Log($"{nameof(InitializeSingletonAsync)}: singleton of type '{typeof(T).Name}' is already initialized.");
			yield break;
		}

		if (_applicationStateIsChanging)
		{
			Debug.LogWarning($"{nameof(InitializeSingletonAsync)}: Failed to initialize because the application state is changing");
			yield break;
		}
		...
		if (typeIsMonoBehaviour)
		{
			...
			if (sceneInstance != null)
			{
				...
				Debug.Log($"{nameof(InitializeSingletonAsync)}: found instance in scene of type '{typeof(T).Name}'", sceneInstance);
				yield break;
			}
		}
		...
		if (attribute == null)
		{
			Debug.LogError($"{nameof(LoadFromResourcesAsync)}: Could not get {nameof(SingletonAttribute)} for type '{typeof(T).Name}'");
			yield break;
		}

		var loadRoutine = attribute.Source switch
		{
			SingletonAttribute.AssetSource.Addressables => LoadFromAddressablesAsync<T>(attribute.Path),
		...
		if (singleton == null)
		{
			Debug.LogError($"{nameof(LoadFromResourcesAsync)}: Failed to load singleton of type '{typeof(T).Name}'");
			yield break;
		}
		...
		if (typeIsMonoBehaviour)
		{
			...
			Debug.Log($"{nameof(LoadFromResourcesAsync)}: Instantiated {typeof(MonoBehaviour).Name} singleton of type '{typeof(T).Name}'");
		}
	...
	public static void DisposeSingleton<T>() where T : UnityObject
	{
		...
		{
			Debug.LogWarning($"{nameof(DisposeSingleton)}: No registered singleton of type '{typeof(T).Name}'");
			return;
		}
		...
		if (singleton is MonoBehaviour monoBehaviour)
		{
			...
			Debug.Log($"{nameof(DisposeSingleton)}: Destroyed singleton game object for type '{typeof(T).Name}'");
		}
	}
	...
	public static bool TryGetSingletonPath<T>(out string path)
	{
		...
		if (attribute == null)
		{
			Debug.LogError($"{nameof(TryGetSingletonPath)}: Could not get {nameof(SingletonAttribute)} for type '{typeof(T).Name}'");
			path = string.Empty;
			return false;
		}
		...
		{
			Debug.LogError($"{nameof(TryGetSingletonPath)}: Path for singleton type '{typeof(T).Name}' is empty");
			path = string.Empty;
			return false;
		}
	...
	public static bool TryLoadSingleton<T>(out T asset) where T : UnityObject
	{
		...
		if (attribute == null)
		{
			Debug.LogError($"{nameof(TryLoadSingleton)}: Could not get {nameof(SingletonAttribute)} for type '{typeof(T).Name}'");
			asset = default;
			return false;
		}
		...
		{
			SingletonAttribute.AssetSource.Addressables => TryLoadFromAddressables(attribute.Path, out asset),
	...
	public static bool TryLoadFromResources<T>(string path, out T asset) where T : UnityObject
	{
		...
		{
			Debug.LogError($"{nameof(TryLoadFromResources)}: empty singleton path for type '{typeof(T).Name}'");
			asset = default;
			return false;
		}
	...
	/// Prefer using <see cref="InitializeSingletonAsync"/> or <see cref="LoadFromAddressablesAsync"/>
	/// </remarks>
	public static bool TryLoadFromAddressables<T>(string path, out T asset) where T : UnityObject
	{
		if (string.IsNullOrEmpty(path))
		{
			Debug.LogError($"{nameof(TryLoadFromAddressables)}: empty singleton path for type '{typeof(T).Name}'");
			asset = default;
			return false;
		}
		...
		if (typeIsMonoBehaviour)
		{
			...
			{
				Debug.LogError($"{nameof(TryLoadFromAddressables)}: handle.Result is not a GameObject; T = {typeof(T).Name}");
				asset = default;
				return false;
			}
		...
		if (asset == null)
		{
			Debug.LogError($"{nameof(TryLoadFromAddressables)}: asset is not type '{typeof(T).Name}'; path = {path}");
			return false;
		}
	...
	public static IEnumerator LoadFromResourcesAsync<T>(string path = "") where T : UnityObject
	{
		...
		{
			Debug.LogError($"{nameof(LoadFromResourcesAsync)}: Could not find {nameof(path)} for type '{typeof(T).Name}'");
			yield break;
		}
	...
	public static IEnumerator LoadFromAddressablesAsync<T>(string path = "") where T : UnityObject
	{
		// Validate our 'path' parameter
		if (string.IsNullOrEmpty(path) && !TryGetSingletonPath<T>(out path))
		{
			Debug.LogError($"{nameof(LoadFromAddressablesAsync)}: Could not find {nameof(path)} for type '{typeof(T).Name}'");
			yield break;
		}
		...
		if (typeIsMonoBehaviour)
		{
			...
			if (handle.Result is not GameObject gameObject)
			{
				Debug.LogError($"{nameof(LoadFromAddressablesAsync)}: handle.Result is not a GameObject; T = {typeof(T).Name}");
				yield break;
			}
